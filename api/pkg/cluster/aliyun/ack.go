...
// New 创建 ACK Provider (暂不实现完整功能)。
func New(cfg cluster.ClusterConfig) (*Provider, error) {
	return &Provider{config: cfg}, nil
}

func (p *Provider) Connect(ctx context.Context) error {
	return cluster.NewError(cluster.KindConnectError, "ACK not implemented yet", nil)
}

func (p *Provider) HealthCheck(ctx context.Context) (*cluster.HealthInfo, error) {
	return nil, cluster.NewError(cluster.KindConnectError, "ACK not implemented yet", nil)
}
...
func (p *Provider) Deploy(ctx context.Context, spec cluster.DeploySpec) (*cluster.DeployResult, error) {
	return nil, cluster.NewError(cluster.KindDeployError, "ACK not implemented yet", nil)
}

func (p *Provider) Rollback(ctx context.Context, namespace, deployment string, toRevision int64) error {
	return cluster.NewError(cluster.KindDeployError, "ACK not implemented yet", nil)
}
